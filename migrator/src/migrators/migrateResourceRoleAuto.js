...
const fs = require('fs');
const { migrateResourceRole } = require('./migrateResourceRole');
const { migrateResourceRole: migrateBatch } = require('./migrateResourceRoleBatch');

const FILE_SIZE_THRESHOLD = 3 * 1024 * 1024; // 3 MB

async function migrateResourceRoleAuto(filePath, startDate) {
  const stats = fs.statSync(filePath);
  const fileSize = stats.size;

  if (fileSize < FILE_SIZE_THRESHOLD) {
    // Using normal migration (in-memory)
    await migrateResourceRole(filePath, startDate);
  } else {
...
module.exports = { migrateResourceRoleAuto };
